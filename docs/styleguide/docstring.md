...
- В теле функций использовать одинарные кавычки `'...'`, кроме docstring.  
- Многострочный docstring:
  - Открывающие `"""` на отдельной строке.
  - С новой строки - краткое описание.
...
- Если функция может выбрасывать исключения — документировать с `:raises`.
...
- Не писать комментарии "для очевидного", только по отдельному запросу или когда действительно нужно пояснить нетривиальную логику.  
- Предпочтение отдаётся чистому коду и хорошим docstring вместо обилия комментариев.

## Кавычки

- В теле функций и тестов — одинарные кавычки `'...'`.  
- В docstring — стандартные тройные двойные кавычки `"""..."""`.
